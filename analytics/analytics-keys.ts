enum EVENTS {
  // navbar events
  CLICK_FAQ = "click_faq",
  CLICK_PRICING = "click_pricing",
  CLICK_HOW_IT_WORKS = "click_how_it_works",
  CLICK_BLOG = "click_blog",
  CLICK_LOGO = "click_logo",

  // send join the beta form
  SEND_JOIN_THE_BETA_FORM = "send_join_the_beta_form",

  // click on pricing plans
  CLICK_PRICING_PLAN_BASE = "click_pricing_plan_base",
  CLICK_PRICING_PLAN_PRO = "click_pricing_plan_pro",
  CLICK_PRICING_PLAN_MAX = "click_pricing_plan_max",

  // click on read more blog banner
  CLICK_READ_MORE_BLOG_BANNER = "click_read_more_blog_banner",

  // click on FAQ content
  CLICK_FAQ_CONTENT_LAUNCH_DATE = "click_faq_content_launch_date",
  CLICK_FAQ_CONTENT_PROJECT_GOAL = "click_faq_content_project_goal",
  CLICK_FAQ_CONTENT_PLANS_AND_PRICING = "click_faq_content_plans_and_pricing",
  CLICK_FAQ_CONTENT_VERIFICATION = "click_faq_content_verification",
  CLICK_FAQ_CONTENT_SUGGESTIONS = "click_faq_content_suggestions",

  // click on privacy policy
  CLICK_PRIVACY_POLICY = "click_privacy_policy",

  // click on Instagram
  CLICK_INSTAGRAM = "click_instagram",
}

export { EVENTS };
